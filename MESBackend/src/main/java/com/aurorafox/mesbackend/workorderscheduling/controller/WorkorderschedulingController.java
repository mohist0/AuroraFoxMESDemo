 ...
 * 工单排程 REST 接口
 * <p>
 * 提供工单排程的增删查改端点，路径统一前缀 {@code /api/workorderscheduling}。
 */
...
public class WorkorderschedulingController {
    ...
    @Operation(summary = "创建工单排程",
            description = "创建一个新的工单排程并返回保存后的排程信息",
    ...
    @Operation(summary = "根据ID获取工单排程",
            description = "通过排程编号查询单个工单排程信息",
            security = {@SecurityRequirement(name = "bearerAuth")})
    ...
    @Operation(summary = "获取所有工单排程",
            description = "查询系统中所有的工单排程列表",
            security = {@SecurityRequirement(name = "bearerAuth")})
    @GetMapping("/getAll")
    ...
    @Operation(summary = "更新工单排程",
            description = "根据排程编号更新工单排程信息，返回更新后的排程",
            security = {@SecurityRequirement(name = "bearerAuth")})
    ...
    @Operation(summary = "删除工单排程",
            description = "根据排程编号删除工单排程，删除成功返回 204 No Content",
            security = {@SecurityRequirement(name = "bearerAuth")})
